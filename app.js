...
$('#loginForm').addEventListener('submit', event => { event.preventDefault(); const data = new FormData(event.target); const username = String(data.get('username') || '').trim(); const password = String(data.get('password') || '').trim(); if (accounts[username] && accounts[username] === password) { sessionStorage.setItem('oakwell-user', username); $('#loginScreen').classList.add('hidden'); $('#appUserName').textContent = username; $('#greetingName').textContent = username; } else $('#loginError').classList.add('visible'); });
