...
const qatarDateParts = () => Object.fromEntries(new Intl.DateTimeFormat('en-US', { timeZone: qatarTimeZone, year: 'numeric', month: 'numeric', day: 'numeric' }).formatToParts(new Date()).filter(part => part.type !== 'literal').map(part => [part.type, Number(part.value)]));
const qatarDateLabel = (daysFromToday = 0) => { const { year, month, day } = qatarDateParts(); const date = new Date(Date.UTC(year, month - 1, day + daysFromToday)); return new Intl.DateTimeFormat('en-GB', { timeZone: 'UTC', day: '2-digit', month: 'short', year: 'numeric' }).format(date); };
const qatarLongDateLabel = () => new Intl.DateTimeFormat('en-GB', { timeZone: qatarTimeZone, weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' }).format(new Date());
const getTomorrowLabel = () => qatarDateLabel(1);
const dueTomorrow = () => books.filter(book => book.status === 'loaned' && book.due === getTomorrowLabel());

function updateStats() {
  $('#overviewDate').textContent = qatarLongDateLabel();
...
function returnBook(bookId) { const book = books.find(item => item.id === bookId); if (!book) return; const borrower = book.borrower; const historyEntry = [...loanHistory].reverse().find(entry => entry.bookId === bookId && !entry.returned); if (historyEntry) { historyEntry.returned = true; historyEntry.returnedOn = qatarDateLabel(); } book.status = 'available'; delete book.borrower; delete book.studentId; delete book.borrowed; delete book.due; activities.unshift({ type: 'return', icon: '↙', title: `${borrower || 'A student'} returned ${book.title}`, meta: `${book.id} · Today`, time: 'Just now' }); save(); renderOverview(); renderLoans(); renderCollection(); renderAudit(); showLookup(bookId); showToast(`${book.title} is back in the library.`); }
...
function closeSidebar() { const sidebar = $('.sidebar'); sidebar.classList.remove('open'); sidebar.style.setProperty('transform', 'translateX(-100%)', 'important'); }
$('#closeSidebar').addEventListener('click', closeSidebar);
function signOut() { sessionStorage.removeItem('oakwell-user'); closeSidebar(); $('#loginScreen').classList.remove('hidden'); $('#loginForm').reset(); $('#loginError').classList.remove('visible'); $('#loginForm input[name="username"]').focus(); }
...
function signIn() { const data = new FormData($('#loginForm')); const username = String(data.get('username') || '').trim(); const password = String(data.get('password') || '').trim(); if (accounts[username] && accounts[username] === password) { const sidebar = $('.sidebar'); sidebar.classList.remove('open'); sidebar.style.removeProperty('transform'); sessionStorage.setItem('oakwell-user', username); $('#loginScreen').classList.add('hidden'); $('#appUserName').textContent = username; $('#greetingName').textContent = username; } else $('#loginError').classList.add('visible'); }
$('#loginForm').addEventListener('submit', event => { event.preventDefault(); signIn(); });
$('#loginForm button[type="submit"]').addEventListener('click', event => { event.preventDefault(); signIn(); });
